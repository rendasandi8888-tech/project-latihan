...
export function useRole(): RoleState {
  ...
  useEffect(() => {
    ...
    async function fetchRole() {
      if (!account?.address) {
        if (!cancelled) setState({ role: 'UNREGISTERED', userProfile: null, isLoading: false, isConnected: false, address: null })
        return
      }

      const cached = roleCache.get(account.address)
      if (cached && Date.now() - cached.fetchedAt < CACHE_TTL) {
        if (!cancelled) setState({ role: cached.role, userProfile: cached.userProfile, isLoading: false, isConnected: true, address: account.address })
        return
      }
      ...
      try {
        ...
        if (!cancelled) setState({ role: roleName, userProfile: profile, isLoading: false, isConnected: true, address: account.address })
      } catch (error) {
        console.error('useRole error:', error)
        if (!cancelled) setState({ role: 'UNREGISTERED', userProfile: null, isLoading: false, isConnected: true, address: account.address })
      } finally {
